...
Current test coverage (as of 2026-07-23): 14 Vitest unit-test files (`robots`, `sitemap`, `page`, `reveal`, `compliance`, `content-integrity`, `ci-path-filter`, `metadata`, `portal-api`, `portal-token`, `portal-verify-api`, `roll-of-honor`, `site-header`, `stripe-api`) and five Playwright specs (`homepage`, `navigation`, `animations`, `giving`, `social-graph`) under `__tests__/` and `e2e/` respectively — grown well past the original "robots/sitemap + one smoke test" baseline as the CI/compliance/content-pipeline/donation work landed. The point was having the harness and CI wiring ready *before* the login/subscription forms land, not retrofitting it after — coverage has kept pace with each addition since.
